...
import { Search, CreditCard, Banknote, Smartphone, RefreshCw, CheckCircle, ArrowLeft, Copy, Lock, Coins, ArrowRightLeft, FileText, User, Star, Gift, X, Printer, Trash2 } from 'lucide-react';
...
export const POS: React.FC = () => {
  ...
  const finalizeSale = async (method: PaymentMethod, isFiscal: boolean = true) => {
    try {
      ...
      const sale = await db.createSale(cart, method, currentClient, discountValue, isFiscal, 'COMPLETED');
    ...
    <div className="h-screen w-full bg-pdv-bg text-white flex flex-col overflow-hidden font-mono">
      ...
      <footer className="bg-slate-900 h-12 flex items-center px-4 justify-between text-sm font-bold text-slate-300 border-t border-slate-800">
        <div className="flex gap-2 overflow-x-auto">
            <button onClick={() => setIsPriceCheckOpen(true)} className="bg-slate-800 px-3 py-1 rounded hover:bg-pdv-accent hover:text-white transition-colors whitespace-nowrap">F5 - CONSULTAR</button>
            <button onClick={handleCancelSale} className="bg-red-700 text-white px-3 py-1 rounded hover:bg-red-600 transition-colors shadow-md font-bold flex items-center gap-1 whitespace-nowrap"><Trash2 size={14} /> F6 - CANCELAR</button>
            <button onClick={handleFastNonFiscalCheckout} className="bg-yellow-600 text-white px-3 py-1 rounded hover:bg-yellow-700 transition-colors shadow-md font-bold flex items-center gap-1 whitespace-nowrap"><Printer size={14} /> F8 - ORÇAMENTO</button>
            <button onClick={() => setIsFunctionsMenuOpen(true)} className="bg-slate-800 px-3 py-1 rounded hover:bg-pdv-accent hover:text-white text-pdv-accent transition-colors whitespace-nowrap">F9 - FUNÇÕES</button>
            <button onClick={startCheckoutFlow} className="bg-pdv-accent text-white px-4 py-1 rounded hover:bg-blue-600 transition-colors shadow-lg shadow-blue-900/50 whitespace-nowrap">F12 - FECHAR</button>
        </div>
      ...
      {isCashOpenModal && ( <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center"><div className="bg-white rounded-xl p-8 w-96 text-center text-slate-800"><h2 className="text-2xl font-bold mb-2">Abertura de Caixa</h2><form onSubmit={handleOpenCash}><input ref={openingInputRef} type="number" step="0.01" className="w-full text-center text-3xl font-bold border-2 border-slate-300 rounded-lg p-2 mb-6 text-slate-900" value={openingValue} onChange={e => setOpeningValue(e.target.value)} placeholder="0,00" /><button type="submit" className="w-full bg-pdv-success text-white font-bold py-3 rounded-lg">CONFIRMAR</button></form></div></div> )}
